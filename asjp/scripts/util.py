...
def parse_metadata(line):
    """
    The second line gives properties of the languages,
    in fixed format separated by blanks (not tabs),
    so the columns are important.
    Col. 2: 3 if the language is the first one in a new
    WALS family, 2 if it’s the first language in a
    new WALS genus, 1 otherwise.
    ...
    Col. 19-30, right justified: number of speakers, from
    Ethnologue
    . This number always refers to
    the entire language, as defined in
    Ethnologue
    , even if the list itself refers to a dialect. The
    number is 0 if the number of speakers is unknown; -
    1 if the language is recently extinct; -2 if the
    language is long extinct; or if the approximate dat
    e of extinction is known, the date is preceded
    by a minus sign. In the ASJP software, if there is
    a date in the first line of the entire file, lists
    with earlier extinction dates here are ignored, as
    are lists with -2; otherwise, all lists are read.
    Col. 34-36: three-letter WALS code, if any.
    Col. 40-42: three-letter ISO639-3 code, if any. Thi
    s code is included for languages in previous
    editions of
    Ethnologue
    even if they aren’t in the 17th edition. Languages
    that lack an ISO639-3
    code but can be placed in the
    Ethnologue
    classification are given a code consisting of two
    letters
    followed by the number 0 (for use in ASJP software)
    .
    """
    return dict(
        latitude=float(line[4:11].strip() or 0),
        longitude=float(line[11:18].strip() or 0),
        population=int(line[19:31].strip() or 0),
        wals=line[34:37],
        iso=line[39:42])
...
COUNTER_PATTERN = re.compile('\_[0-9]+$')
...
def parse(fp):
    ...
    for line in fp:
        ...
        if m:
            ...
            language['contribution'] = language['name']
            if COUNTER_PATTERN.search(language['name']):
                language['name'] = ''.join(language['name'].split('_')[:-1])
            language['words'] = {}
            in_header = True
